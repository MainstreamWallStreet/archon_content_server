...
from src.models import Item, ItemCreate, ItemUpdate, ObjectListResponse, ObjectUploadRequest, ObjectDownloadResponse
...
class TestAuthentication:
    ...
    def test_authenticated_endpoint_with_valid_key(self, test_client: TestClient, api_key_header: dict):
    ...
    def test_authenticated_endpoint_with_invalid_key(self, test_client: TestClient, invalid_api_key_header: dict):
...
class TestItemsEndpoints:
    ...
    def test_list_items_store_error(self, test_client: TestClient, api_key_header: dict):
    ...
    def test_get_item_not_exists(self, test_client: TestClient, api_key_header: dict):
        ...
        with patch("src.api.get_setting", return_value="test-api-key"):
            with patch("src.api.data_store") as mock_store:
                ...
                response = test_client.get("/items/non-existent", headers=api_key_header)
    ...
    def test_create_item_invalid_data(self, test_client: TestClient, api_key_header: dict):
    ...
    def test_create_item_already_exists(self, test_client: TestClient, api_key_header: dict):
    ...
    def test_update_item_not_exists(self, test_client: TestClient, api_key_header: dict):
    ...
    def test_delete_item_not_exists(self, test_client: TestClient, api_key_header: dict):
        """Test deleting a non-existent item."""
        with patch("src.api.get_setting", return_value="test-api-key"):
            with patch("src.api.data_store") as mock_store:
                mock_store.delete_item.side_effect = ValueError("Item not found")
                
                response = test_client.delete("/items/non-existent", headers=api_key_header)
                assert response.status_code == 404
                data = response.json()
                assert "detail" in data
    ...
    def test_run_scheduler_success(self, test_client: TestClient, api_key_header: dict):
        ...
        with patch("src.api.get_setting", return_value="test-api-key"):
            with patch("src.api.scheduler") as mock_scheduler:
                ...
                response = test_client.post("/admin/tasks/run-scheduler", headers=api_key_header)
                assert response.status_code == 200
                data = response.json()
...
class TestObjectStorageEndpoints:
    """Test object storage RESTful API endpoints."""

    @patch("src.api.get_setting", return_value="test-api-key")
    @patch("src.api.get_data_store")
    def test_list_objects(self, mock_get_store, mock_get_setting, test_client: TestClient, api_key_header: dict):
        mock_store = mock_get_store.return_value
        mock_store.list_objects.return_value = ["foo.txt", "bar.txt"]
        response = test_client.get("/objects", headers=api_key_header)
        assert response.status_code == 200
        data = response.json()
        assert data["objects"] == ["foo.txt", "bar.txt"]

    @patch("src.api.get_setting", return_value="test-api-key")
    @patch("src.api.get_data_store")
    def test_upload_object(self, mock_get_store, mock_get_setting, test_client: TestClient, api_key_header: dict):
        mock_store = mock_get_store.return_value
        obj_name = "test.txt"
        content = b"hello world"
        encoded = base64.b64encode(content).decode()
        req = {"object_name": obj_name, "data": encoded}
        response = test_client.post("/objects", headers=api_key_header, json=req)
        assert response.status_code == 201
        mock_store.put_object.assert_called_once_with(obj_name, content)

    @patch("src.api.get_setting", return_value="test-api-key")
    @patch("src.api.get_data_store")
    def test_upload_object_already_exists(self, mock_get_store, mock_get_setting, test_client: TestClient, api_key_header: dict):
        mock_store = mock_get_store.return_value
        mock_store.put_object.side_effect = FileExistsError("exists")
        obj_name = "test.txt"
        content = b"hello world"
        encoded = base64.b64encode(content).decode()
        req = {"object_name": obj_name, "data": encoded}
    ...
    @patch("src.api.get_setting", return_value="test-api-key")
    @patch("src.api.get_data_store")
    def test_download_object(self, mock_get_store, mock_get_setting, test_client: TestClient, api_key_header: dict):
        mock_store = mock_get_store.return_value
        obj_name = "test.txt"
        content = b"hello world"
        mock_store.get_object.return_value = content
        response = test_client.get(f"/objects/{obj_name}", headers=api_key_header)
        assert response.status_code == 200
        data = response.json()
        assert data["object_name"] == obj_name
        assert base64.b64decode(data["data"]) == content

    @patch("src.api.get_setting", return_value="test-api-key")
    @patch("src.api.get_data_store")
    def test_download_object_not_found(self, mock_get_store, mock_get_setting, test_client: TestClient, api_key_header: dict):
        mock_store = mock_get_store.return_value
        mock_store.get_object.side_effect = FileNotFoundError("not found")
        response = test_client.get("/objects/missing.txt", headers=api_key_header)
        assert response.status_code == 500 or response.status_code == 404

    @patch("src.api.get_setting", return_value="test-api-key")
    @patch("src.api.get_data_store")
    def test_update_object(self, mock_get_store, mock_get_setting, test_client: TestClient, api_key_header: dict):
        mock_store = mock_get_store.return_value
        obj_name = "test.txt"
        content = b"updated"
        encoded = base64.b64encode(content).decode()
        req = {"object_name": obj_name, "data": encoded}
        response = test_client.put(f"/objects/{obj_name}", headers=api_key_header, json=req)
        assert response.status_code == 200
        mock_store.update_object.assert_called_once_with(obj_name, content)

    @patch("src.api.get_setting", return_value="test-api-key")
    @patch("src.api.get_data_store")
    def test_update_object_not_found(self, mock_get_store, mock_get_setting, test_client: TestClient, api_key_header: dict):
        mock_store = mock_get_store.return_value
        mock_store.update_object.side_effect = FileNotFoundError("not found")
    ...
    @patch("src.api.get_setting", return_value="test-api-key")
    @patch("src.api.get_data_store")
    def test_delete_object(self, mock_get_store, mock_get_setting, test_client: TestClient, api_key_header: dict):
        mock_store = mock_get_store.return_value
        obj_name = "test.txt"
        response = test_client.delete(f"/objects/{obj_name}", headers=api_key_header)
        assert response.status_code == 200
        mock_store.delete_object.assert_called_once_with(obj_name)

    @patch("src.api.get_setting", return_value="test-api-key")
    @patch("src.api.get_data_store")
    def test_delete_object_not_found(self, mock_get_store, mock_get_setting, test_client: TestClient, api_key_header: dict):
        mock_store = mock_get_store.return_value
        mock_store.delete_object.side_effect = FileNotFoundError("not found")
        obj_name = "missing.txt"
        response = test_client.delete(f"/objects/{obj_name}", headers=api_key_header)
        assert response.status_code == 500 or response.status_code == 404 