...
class NotificationAgent:
    ...
    def send(self, subject: str, body: str) -> None:
        logger.info("Attempting to send email to %s with subject: %s", self.recipient, subject)
        ...
        try:
            ...
            if resp.status_code >= 400:
                logger.error("SendGrid error %s for recipient %s: %s", resp.status_code, self.recipient, resp.body)
                raise RuntimeError(f"SendGrid error {resp.status_code}: {resp.body}")
            logger.info("✅ Successfully sent email to %s (status: %s)", self.recipient, resp.status_code)
...
def send_alert(subject: str, message: str) -> None:
    ...
    try:
        ...
        if not isinstance(recipients, list):
            logger.error("ALERT_RECIPIENTS must be a JSON array, got: %s", type(recipients).__name__)
            raise RuntimeError(f"ALERT_RECIPIENTS must be a JSON array, got: {type(recipients).__name__}")
        
        logger.info("📧 Found %d recipient(s) for alert distribution: %s", len(recipients), recipients)
        
        if not recipients:
            logger.warning("⚠️  No recipients configured in ALERT_RECIPIENTS - alert will not be sent")
            return
    ...
    logger.info("✅ Alert distribution complete: %d successful, %d failed out of %d total recipients", 
                successful_sends, failed_sends, len(recipients))
    
    if failed_sends > 0:
        logger.warning("⚠️  %d email(s) failed to send - check logs above for details", failed_sends)
