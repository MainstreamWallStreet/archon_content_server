...
def test_send_email_success():
    ...
    with patch("src.notifications.SendGridAPIClient", return_value=fake_client), patch(
        ...
        log.info.assert_any_call(
            "✅ Successfully sent email to %s (status: %s)", "user@example.com", 202
        )
...
def test_send_alert_no_recipients():
    ...
    log = MagicMock()
    with patch("src.notifications.get_setting", side_effect=_settings), patch(
        "src.notifications.logger", log
    ):
        send_alert("subj", "body")
...
def test_send_alert_partial_failure():
    ...
    with patch("src.notifications.SendGridAPIClient", return_value=fake_client), patch(
        ...
        log.warning.assert_any_call(
            "⚠️  %d email(s) failed to send - check logs above for details", 1
        )
